...
async function onSubmit(values: RegisterModel) {
  loading.value = true
  try {
    ...
    useFaToast().success('注册成功', { description: '请登录您的账号' })
    emits('onRegister', values.account)
  }
  catch (error: any) {
    console.error('[Register] error:', error)
    useFaToast().error('注册失败', { description: error?.data?.message || error?.message || '注册失败，请稍后重试' })
  }
  finally {
    loading.value = false
  }
